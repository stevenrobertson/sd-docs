...
the continuous domain must be sampled along the 2D grid of pixels used in raster 
graphics.
...
downfilter the highest spatial frequency components below the sampling threshold. 
Unfortunately, downfiltering any image component, especially surface textures, 
results in a reduction of detail across the image. Our brains notice artifacts 
from aliasing at object borders, but also notice reduced detail apart from those 
regions.
...
front of your face, does it look blurry?  Yes, it does.  This is the way we 
percieve the world and it doesn't usually occur to us consciously, our brain 
takes care of deblurring the image for us.  In fact, it is so much a part of the 
way we percieve the world such that if we didn't see a motion blur, the moving 
